...
namespace Iris.ParserUtils
{
    public class LexParsa
    {
        ...
        private readonly static Regex elseIfRegex = new Regex("^" + @elseIfExp + @"[\W]");
        ...
        public void lexCode()
        {
            ...
            switch (codeToParse[0])
            {
                ...
                default:
                    ...
                    if (intRegex.IsMatch(codeToParse))
                    {
                        lexInt();
                        return;
                    }
                    ...
                    else if (elseIfRegex.IsMatch(codeToParse))
                    {
                    ...
                    {
                        System.Windows.MessageBox.Show("Probleme ici: " + codeToParse[0]);
                        throw new InvalidSequence(lineNumber, colNumber);
                    }
        ...
        public bool startParse(string code)
        {
            /*Lexer l = new Lexer();
            ...
            {
                Print.Printer.println(tok.ToString());
            }*/
        ...
        public bool startParse()
        {
            ...
            {
                //Print.Printer.printError("[Exception]");
                //IrisPrinter.Printer.printError(e.ExceptionToString());
        ...
        {
            switch (lexBuf.ElementAt(indiceToken).type)
            {
            ...
            while (!parseEnd)
            {
                switch (currentToken.type)
                {
                    ...
                    case Token.TokenValue.MSG:
                        ...
                        //System.Windows.MessageBox.Show("ParseSimple");
                    ...
                    case Token.TokenValue.PLAY:
                        ...
                        if (currentTokenEquals(Token.TokenValue.VALUE_IDENTIFIER))
                        {
                            //play.path = new VariableExpression(currentToken.stringValue);
                            ...
                            play.loop = parseLiteralExpression();

                            if (play.loop.getExpressionType() != ObjectType.booleanType)
                            {
                                throw new IncompatibleTypesException(lineNumber, colNumber, ObjectType.booleanType, play.loop.getExpressionType());
                            }*/
                        }
                        else
                        {
                    ...
                    case Token.TokenValue.SHOW:
                        ...
                        if (currentTokenEquals(Token.TokenValue.VALUE_IDENTIFIER))
                        {
                            ...
                            //show.position = parseSimpleExpression();
                            //TODO
                            if (currentTokenEquals(Token.TokenValue.VALUE_IDENTIFIER))
                            {
                    ...
                    case Token.TokenValue.SWITCHCHAR:
                        ...
                        /*
                         * 
                         * if(!Data.characters.ContainsKey(switchChar.character.name){
                         *      //throw new IncompatibleTypesException(lineNumber, colNumber, ObjectType.characterType, ...);
                         * }
                         * 
                         * */
            ...
            while (!parseEnd)
            {
                switch (currentToken.type)
                {
                    ...
                    case Token.TokenValue.BOOLEAN:
                        /*exps.Push(new LiteralExpression(currentToken));
                        nextToken();*/
                    ...
                    case Token.TokenValue.EQBOOL:
                        ...
                        if (e1 is BinaryOperatorExpression)
                        {
                            left = (BinaryOperatorExpression)e1;

                            //(a && b) <= c --> a && (b <= c)
                            //(left.e1 && left.e2) <= c --> left.e1 && (left.e2 <= c)
                            if (tokenPriority(currentToken.type) < tokenPriority(left.value))
                            {
                                //e1 = (SimpleExpression)left.e1;
                                e1 = new BinaryOperatorExpression(Token.TokenValue.AND, null, null);
                        ...
                        {
                            right = (BinaryOperatorExpression)e2;

                            //a <= (b && c) --> (a <= b) && c
                            //e1 <= (e2.e1 && e2.e2) --> (e1 <= e2.e1) && e2.e2
                            if (tokenPriority(currentToken.type) > tokenPriority(right.value))
                            {

                            }
                        }
                        
                        exps.Push(new BinaryOperatorExpression(currentToken.type, e1, e2));*/
        ...
        private PathExpression parsePathExpression()
        {
            ...
            if (currentTokenEquals(Token.TokenValue.VALUE_IDENTIFIER))
            {
                ...
                switch (currentToken.type)
                {
                    /*case Token.TokenValue.DOT:
                        nextToken();

                        return new SubMemberExpression(valueName, parsePathExpression());*/
                    ...
                    case Token.TokenValue.LBRACKET:
                        ...
                        if (currentTokenEquals(Token.TokenValue.STRING))
                        {
                             ...
                             * }
                             *              OU
                             * if(!Data.characters.ContainsKey(valueName)){
                             *      
                             * }
                             * 
                             * //---CLE EXISTANTE---
                             * 
                             * if(!Data.characters[valueName].images.ContainsKey(current
                             * 
                             *              OU
                             * 
                             * if(variables[valueName].type != ObjectType.characterType){
                             *      throw new IncompatibleTypesException(lineNumber, colNumber, ObjectType.characterType, variables[valueName].type);
                             * }
                             * if(!(IrisCharacter)variables[valueName]
                             * 
                             * */
        ...
        public AssignExpression parseAssignExpression()
        {
            ...
            if (currentTokenEquals(Token.TokenValue.VALUE_IDENTIFIER))
            {
                ...
                //assignExp.path = new VariableExpression(varName);
        ...
        /*public bool nextTokenEquals(Token.TokenValue value)
        {
            if (indiceToken + 1 >= lexBuf.Count)
            {
                //erreur
                return false;
                //throw new Exception();
            ...
            return (lexBuf.ElementAt(indiceToken + 1).type == value);
        }*/
